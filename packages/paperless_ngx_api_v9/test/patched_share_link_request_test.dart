import 'package:test/test.dart';
import 'package:paperless_ngx_api_v9/paperless_ngx_api_v9.dart';

// tests for PatchedShareLinkRequest
void main() {
  final PatchedShareLinkRequest? instance = /* PatchedShareLinkRequest(...) */
      null;
  // TODO add properties to the entity

  group(PatchedShareLinkRequest, () {
    // DateTime expiration
    test('to test the property `expiration`', () async {
      // TODO
    });

    // int document
    test('to test the property `document`', () async {
      // TODO
    });

    // FileVersionEnum fileVersion
    test('to test the property `fileVersion`', () async {
      // TODO
    });
  });
}
