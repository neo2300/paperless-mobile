# paperless_ngx_api_v9.api.GroupsApi
...
final api = PaperlessNgxApiV9().getGroupsApi();
final GroupRequest groupRequest = ; // GroupRequest | 

try {
    final response = api.groupsCreate(groupRequest);
    print(response);
...
try {
    api.groupsDestroy(id);
} catch on DioException (e) {
    print('Exception when calling GroupsApi->groupsDestroy: $e\n');
}
```
...
# **groupsList**
> PaginatedGroupList groupsList(nameIcontains, nameIendswith, nameIexact, nameIstartswith, ordering, page, pageSize)
...
try {
    final response = api.groupsList(nameIcontains, nameIendswith, nameIexact, nameIstartswith, ordering, page, pageSize);
    print(response);
} catch on DioException (e) {
...
# **groupsPartialUpdate**
> Group groupsPartialUpdate(id, patchedGroupRequest)
...
final api = PaperlessNgxApiV9().getGroupsApi();
final int id = 56; // int | A unique integer value identifying this group.
final PatchedGroupRequest patchedGroupRequest = ; // PatchedGroupRequest | 

try {
    final response = api.groupsPartialUpdate(id, patchedGroupRequest);
    print(response);
} catch on DioException (e) {
...
try {
    final response = api.groupsRetrieve(id);
    print(response);
} catch on DioException (e) {
    print('Exception when calling GroupsApi->groupsRetrieve: $e\n');
}
```
...
# **groupsUpdate**
> Group groupsUpdate(id, groupRequest)
...
try {
    final response = api.groupsUpdate(id, groupRequest);
    print(response);
} catch on DioException (e) {
