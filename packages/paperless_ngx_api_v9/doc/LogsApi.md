# paperless_ngx_api_v9.api.LogsApi
...
[**retrieveLog**](LogsApi.md#retrievelog) | **GET** /api/logs/{id}/ | 
...
final api = PaperlessNgxApiV9().getLogsApi();

try {
    final response = api.logsList();
    print(response);
} catch on DioException (e) {
    print('Exception when calling LogsApi->logsList: $e\n');
}
```
...
try {
    final response = api.retrieveLog(id);
    print(response);
} catch on DioException (e) {
    print('Exception when calling LogsApi->retrieveLog: $e\n');
}
```
